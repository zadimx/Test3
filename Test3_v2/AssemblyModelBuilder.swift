...
protocol AssemblyNewsBuilderProtocol {
  ...
  func createDateilNewsModuleViewController(data: Articles, router: RouterProtocol) -> UIViewController
...
class AssemblyModelBuilder: AssemblyNewsBuilderProtocol {
  ...
  func createMainNewsModuleTableViewCell(router: RouterProtocol, tableView: UITableView, indexPath: IndexPath) -> MainNewsCollectionViewTableViewCell {
    ...
    let mainNewsPresenter = MainNewsCollerctionViewTableViewCellPresenter(view: cell!, networkService: networkService, router: router, endpoint: HardCode.endpointUrlMainNews)
